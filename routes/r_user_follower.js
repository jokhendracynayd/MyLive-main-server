...
router.route("/follow-unfollow").post(
  ...
  asyncErrorHandler(async (req, res, next) => {
    ...
    if (follower_userID && primary_userId) {
      ...
      if (isUser.length == 0) {
        ...
        // make un follow
        const ok = await TableModel.Table.findByIdAndUpdate(
          isUser[0]._id,
          { $set: { status: !isUser[0].status } },
          { new: true }
        ).exec();
        if (ok) {
          if (!isUser[0].status) {
...
router.route("/friends-list/:UID").get(
  asyncErrorHandler(async (req, res) => {
    const UID = req.params.UID;
    if (!UID) {
      return res.json({
    ...
    let usersWithDetails = await TableModelLogin.Table.find(
      { UID: { $in: userIds } },
      { user_nick_name: 1, user_profile_pic: 1, level: 1, vip: 1, UID: 1 }
    );
    // Check profile pic is url and update
    usersWithDetails = usersWithDetails.map((user) => {
      return {
        user_profile_pic: user.user_profile_pic.startsWith("http")
          ? user.user_profile_pic
          : `${API.SocketAPI}/users/get-profile-pic/${user.UID}`,
...
router.route("/followers-followings/:UID").get(
  asyncErrorHandler(async (req, res) => {
    const UID = req.params.UID;
    if (!UID) {
      return res.json({
...
router.route("/followers-list/:UID").get(
  asyncErrorHandler(async (req, res) => {
    const UID = req.params.UID;
    if (!UID) {
      return res.json({
...
router.route("/followings-list/:UID").get(
  asyncErrorHandler(async (req, res) => {
    const UID = req.params.UID;
    if (!UID) {
      return res.json({
