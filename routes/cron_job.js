...
cron.schedule('0 0 * * *',async()=>{
  const fruitProfit = await redisClient.get('fruitGameRev');
  const teenPattiGameRev = await redisClient.get("teenPattiGameRev");
  const greedyGameRev = await redisClient.get("greedyGameRev");
  console.log(fruitProfit,teenPattiGameRev);
  ...
  const isCreated = await new GameProfitTable({
    date:formattedDate,
    fruit_game_profit:fruitProfit || 0,
    teenPatti_game_profit:teenPattiGameRev || 0,
    greedy_game_profit:greedyGameRev || 0
  }).save();
  if(isCreated){
    await redisClient.set('fruitGameRev',0);
    await redisClient.set('teenPattiGameRev',0);
    await redisClient.set('greedyGameRev',0);
    return;
  }
})
